...
print ('Computing mask power spectra ... ')
...
print ('Computing sigma for l1 = l2 = ', l1use_a)

sigma_flat = zeros(len(L_insum))
sigma_full = zeros(len(L_insum))
sigma_node = zeros(len(L_insum))
for i in range(len(L_insum)):
    sigma_flat[i] = sigma_l1l2_L_Limber(l1use_a, l2use_a, L_insum[i])
    sigma_full[i] = sigma_l1l2_L_full(l1use_a, l2use_a, L_insum[i])
    sigma_node[i] = sigma_l1l2_L_node(l1use_a, l2use_a, L_insum[i])
...
fout = open('data_sigma_flat_a.dat', 'w')
for i in range(len(L_insum)):
    fout.write(str(sigma_flat[i])); fout.write('\n')
fout.close()
fout = open('data_sigma_full_a.dat', 'w')
for i in range(len(L_insum)):
    fout.write(str(sigma_full[i])); fout.write('\n')
fout.close()
fout = open('data_sigma_node_a.dat', 'w')
for i in range(len(L_insum)):
    fout.write(str(sigma_node[i])); fout.write('\n')
fout.close()

fout = open('data_cls_mask.dat', 'w')
for i in range(len(L_insum)):
    for j in range(len(ffsky)):
        fout.write(str(ccl_masks[i,j])); fout.write(' ');
    fout.write('\n')
fout.close()

...
print 'Computing sigma for l1 = l2 = ', l1use_b
...
for i in range(len(L_insum)):
    print i
    sigma_flat[i] = sigma_l1l2_L_Limber(l1use_b, l2use_b, L_insum[i])
    sigma_full[i] = sigma_l1l2_L_full(l1use_b, l2use_b, L_insum[i])
    sigma_node[i] = sigma_l1l2_L_node(l1use_b, l2use_b, L_insum[i])
...
fout = open('data_sigma_flat_b.dat', 'w')
for i in range(len(L_insum)):
    fout.write(str(sigma_flat[i])); fout.write('\n')
fout.close()
fout = open('data_sigma_full_b.dat', 'w')
for i in range(len(L_insum)):
    fout.write(str(sigma_full[i])); fout.write('\n')
fout.close()
fout = open('data_sigma_node_b.dat', 'w')
for i in range(len(L_insum)):
